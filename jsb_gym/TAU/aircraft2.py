...
class F16(object):
    ...
    def load_PID(self):
        ...
        P = self.conf.pid_rudder_psi['P']
        I = self.conf.pid_rudder_psi['I']
        D = self.conf.pid_rudder_psi['D']      
        self.pid_rudder_psi = PID(P=P, I=I, D=D, Derivator=0, Integrator=0, Integrator_max=10, Integrator_min=-10)        
    ...
    def set_yaw_PID(self, psi_ref):
        
        psi_now = self.get_psi(in_deg=True)  # 获取当前 yaw 角（弧度）
        psi_error = normalize_angle_rad(psi_ref - psi_now)

        rudder_cmd = self.pid_rudder_psi.update(psi_error)

        self.fdm['fcs/rudder-cmd-norm'] = rudder_cmd
    ...
    def set_heading_PID(self, ref_heading, dive_alt):
        ...
        if abs(diff) >= self.heading_clip and abs(diff_alt) <= self.conf.ctrl['alt_act_space']:
            ...
            # print('ref_altitude:', dive_alt)
            # print('get_altitude:', self.get_altitude())
            theta_ref = np.degrees(np.arctan2(diff_atl, self.conf.ctrl['tan_ref']))
    ...
    def step_BVR(self, action, action_type):
        ...
        Note
        ----
        It is required that the aircraft is flying horizontally when initialized.

        Parameters
        ----------
        action : np.ndarray of shape (3,)
            A 3-element array containing:
                - action[0]: heading in [-1, 1] range → mapped to [-180, 180] deg
                - action[1]: altitude in [-1, 1] range → mapped to [3000, 12000] m
                - action[2]: throttle in [-1, 1] range → mapped to [0.0, 1.0]

        action_type : int
            Type of action interpretation.
        """
